      ...
      <main className="py-12">
        <Container>
          ...
          {loading ? (
            ...
            <>
              {/* Desktop Table */}
              <div className="hidden md:block overflow-x-auto rounded-3xl border border-white/5 bg-[#0a1222]/40 backdrop-blur-md shadow-2xl">
              ...
              <div className="space-y-4 md:hidden">
                {filteredPosts.map((post) => {
                  const commentsCount = post.comments?.length || 0;
                  return (
                    <div key={post._id} className="p-5 rounded-3xl border border-white/5 bg-[#0a1222]/40 backdrop-blur-md shadow-lg space-y-4">
                      {/* Header: Cover image & Title */}
                      <div className="flex items-start gap-4">
                        <img
                          src={post.coverImage || "https://via.placeholder.com/80"}
                          alt={post.title}
                          className="w-14 h-14 object-cover rounded-2xl border border-white/10 bg-white/5 flex-shrink-0"
                        />
                        <div className="min-w-0 flex-1">
                          ...
                          <h3 className="font-bold text-white text-base leading-snug line-clamp-2" title={post.title}>
                            {post.title}
                          </h3>
                      ...
                      <div className="flex items-center justify-between gap-4 pt-3 border-t border-white/5">
                        ...
                        <button
                          onClick={() => handleToggleFeatured(post)}
                          className="flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-slate-400 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 hover:text-amber-400 transition"
                        >
                          <svg
                            className={`w-4 h-4 ${post.featured ? "text-amber-400 fill-current" : "text-slate-500"}`}
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2.5"
                            viewBox="0 0 24 24"
                          ...
                          {post.featured ? "Spotlighted" : "Spotlight"}
                        </button>
                      </div>
                      ...
                      <div className="flex gap-2 pt-3 border-t border-white/5">
                        <button
                          onClick={() => navigate(`/admin/community/edit/${post._id}`)}
                          className="flex-1 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-[#40e0d0]/30 hover:bg-[#40e0d0]/10 text-slate-300 hover:text-[#40e0d0] text-xs font-bold flex items-center justify-center gap-2 transition duration-200"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        ...
                        <button
                          onClick={() => handleDelete(post._id)}
                          className="flex-1 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-[#ef4444]/30 hover:bg-[#ef4444]/10 text-slate-300 hover:text-[#ef4444] text-xs font-bold flex items-center justify-center gap-2 transition duration-200"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          </svg>
