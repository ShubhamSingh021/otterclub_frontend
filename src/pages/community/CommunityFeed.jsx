...
const CommunityFeed = () => {
  ...
  const handleLikeToggle = async (postId, e) => {
    ...
    if (!userToken) {
      alert("Please log in to like posts and engage with the community!");
      return;
    }
  ...
  return (
    <AppShell>
      <div className="pt-24 pb-20 min-h-screen bg-[#050b16] text-white">
        ...
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#40e0d0]/10 rounded-full blur-[120px] pointer-events-none" />
        ...
        <SectionWrapper id="community-feed">
            ...
            <form onSubmit={handleSearchSubmit} className="flex flex-col md:flex-row gap-4 justify-between items-center">
              ...
              <div className="relative w-full md:max-w-md">
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 rounded-full border border-white/10 bg-[#050b16]/80 text-white focus:outline-none focus:border-[#40e0d0] transition duration-300"
                />
              ...
              <div className="flex w-full md:w-auto items-center gap-4 justify-end">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="py-2.5 px-4 rounded-full border border-white/10 bg-[#050b16] text-sm text-slate-200 focus:outline-none focus:border-[#40e0d0] cursor-pointer"
                  >
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-full bg-gradient-to-r from-[#2d61ff] to-[#40e0d0] text-sm font-bold text-white hover:shadow-[0_0_15px_rgba(64,224,208,0.4)] transition duration-300"
                >
                  Search
                </button>
              </div>
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-xs font-bold transition duration-300 ${
                  activeCategory === cat
                    ? "bg-[#40e0d0] text-black shadow-[0_0_12px_rgba(64,224,208,0.3)]"
          ...
          {loading ? (
            <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="animate-pulse flex flex-col rounded-[2rem] border border-white/5 bg-[#0a1222] h-[450px]">
                  <div className="h-60 bg-white/5 rounded-t-[2rem]" />
                ...
                return (
                  <a
                    ...
                    className={`group relative flex flex-col overflow-hidden rounded-[2rem] border transition duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(0,0,0,0.4)] ${
                      isFeatured
                        ? "border-[#40e0d0]/40 bg-[#0c1b38] shadow-[0_0_20px_rgba(64,224,208,0.1)]"
                        : "border-white/10 bg-[#0a1222]"
                    } hover:border-[#40e0d0]/50`}
                  >
                    
                    {/* Featured label tag */}
                    {isFeatured && (
                      <div className="absolute left-6 top-6 z-10 flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#40e0d0] to-[#2d61ff] px-3.5 py-1 text-[10px] font-black uppercase tracking-widest text-black shadow-lg">
                    ...
                    <div className="flex flex-grow flex-col p-6 lg:p-8">
                      ...
                      <h3 className="font-display text-2xl font-bold leading-tight text-white group-hover:text-[#40e0d0] transition-colors line-clamp-1">
                        {post.title}
                      </h3>
                      ...
                      <div className="mt-5 flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-gradient-to-r from-[#2d61ff] to-[#40e0d0] flex items-center justify-center text-[9px] font-black text-black">
                          OS
                        </div>
